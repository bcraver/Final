...
Saved a copy of raw data in JJ's public ee282 file just in case.   
...
![](https://github.com/bcraver/Final/blob/master/output/reports/fastqc_forward.png)  
**Figure 1.** FastQC per base sequencing quality for forward reads fastq file.  
![](https://github.com/bcraver/Final/blob/master/output/reports/fastqc_reverse.png)  
**Figure 2.** FastQC per base sequencing quality for reverse reads fastq file.  
...
### Demultiplexing 
To remove barcodes and primers. The following script is saved as demux.sh under the analysis/scripts/ directory. Submitted as a job via qsub
...
# R scripts  
First, visualize number of reads per sample. Then rarefy to normalize the reads. Then use multiplot to identify correlations between Prevotella and individual inflammatory cytokines. 
...
![](https://github.com/bcraver/Final/blob/master/output/figures/alpha_diversity.png)  
**Figure 3.** Alpha diversity in Control (mock sample), healthy individuals and MPN patients. Visulization on qiime2 website.  
![](https://github.com/bcraver/Final/blob/master/output/figures/alpha_diversity_stats.png)  
**Figure 4.** Kruskal-Wallis statistics reveals no significant difference between alpha diversity in healthy individuals and MPN patients (p=0.799647)
![](https://github.com/bcraver/Final/blob/master/output/figures/Rplot_reads_per_sample.png)   
**Figure 5.** Number of reads per sample. Lowest number of reads for an individual (displayed on x-axis) was 2270.
![](https://github.com/bcraver/Final/blob/master/output/figures/Rplot_rarefy.png)   
**Figure 6.** Rarefy to normalize reads to 2200 per sample. 
![](https://github.com/bcraver/Final/blob/master/output/figures/Rplot_nmds.png)  
**Figure 7.** Nonmetric multidimensional scaling (NMDS) using Bray-Curtis dissimilarity.  
![](https://github.com/bcraver/Final/blob/master/output/figures/Rplot_cytokines.png)  
**Figure 8.** Correlations between _Prevotella copri_ and individual cytokines in MPN patients and healthy individuals. There is a greater abundance of _Prevotella copri_ in MPN patients compared to healthy individuals (data not shown).

### Methods: Sequencing and statistical analyses
The purpose of this experiment is to determine whether the microbiome in patients with Myeloproliferative Neoplasm (MPN) is different than healthy aged-matched individuals. We also performed a Luminex assay on each individual's plasma to measure the leves of 11 inflammatory cytokines. 16S rRNA amplicon PCR was performed targeting the full V4-V5 region using the EMP primers (515F (barcoded) and 926R) (Caporaso et al. (2012), Walters et al. (2016)). The library was sequenced at the UC Irvine Genomics High Throughput Facility using a miseq v3 chemistry with a PE300 sequencing length. Sequencing resulted in 16.6 M reads passing filter (of which 25.8% are PhiX) with an overall >Q30 79.9.9%. Raw sequence data were imported into qiime2 (qiime2.org) and demultiplexed on the HPC.

R studio was used to rarefy, plot NMDS dissimilarity and calculate significance between groups and factors and look for correlations between cytokines and taxa within MPN and healthy groups.  

### Conclusions  
Here, we investigated the microbiome of individuals with Myeloproliferative Neoplasm (MPN), a chronic hematological malignancy. Chronic inflammation is common in MPN patients and promotes disease progression. Dysbiosis is implicated in several chronic inflammatory diseases including inflammatory bowel disease, systemic lupus erythematosus, and rheumatoid arthritis. Microbiota can maintain an inflammatory environment by triggering immune cells to release cytokines. Thus, we hypothesized that gut microbiota might play a role in the chronic inflammatory state of MPN patients as well as in their symptom burden, which is correlated with higher plasma inflammatory cytokines. Overall, there was no marked difference in the alpha diversity between healthy individuals and MPN patients. Though the abundance of _Prevotella copri_ was elevated in MPN patients compared to healthy controls, there was no significant correlation between _Prevotella copri_ and individual inflammatory cytokines. Taken together, this pilot study provides evidence that the microbiome is not markedly different in MPN patients compared to healthy individuals. Given that a small number of patients were assayed, further experiments  would be necessary to understand how the microbiota is altered upon disease progression or following different treatment strategies.   
